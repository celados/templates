...
export const createAuthOptions = (ctx: GenericCtx<DataModel>) => {
	const allowedHosts = requireEnvironment('AUTH_ALLOWED_HOSTS')
		.split(',')
		.map((host) => host.trim())
		.filter(Boolean)
	if (allowedHosts.length === 0) {
		throw new Error('AUTH_ALLOWED_HOSTS must include at least one host')
	}

	return {
		// Better Auth resolves each site's callback URL from the validated request
		// host, so one Convex identity store does not imply one canonical frontend.
