...
const siteId = v.union(v.literal('alpha'), v.literal('beta'))
...
export const createSubscriptionCheckout = action({
	...
	handler: async (ctx, args) => {
		const identity = await requireIdentity(ctx)
		const siteUrl = getSiteUrl(args.siteId)
...
export const createCustomerPortal = action({
	...
	handler: async (ctx, args) => {
		...
		return stripe.createCustomerPortalSession(ctx, {
			customerId: customer.customerId,
			returnUrl: getSiteUrl(args.siteId),
		})
	},
})

function getSiteUrl(selectedSite: 'alpha' | 'beta') {
	return requireEnvironment(
		selectedSite === 'alpha' ? 'SITE_ALPHA_URL' : 'SITE_BETA_URL',
	)
}
